...
+ =ensembldb= package defines the =EnsDb= class:
  ...
  - Designed for Ensembl: gene and transcript biotypes.
...
+ Reduced layout
  ...
  #+BEGIN_SRC ditaa :file dblayout-simple.png


    +----------+    +---------+     +-------+     +--------+
    | exons    |----| tx2exon |--+--| tx    |--+--| gene   |
    +----------+    +---------+  |  +-------+  |  +--------+
                                 |             |
                                 |             |  +------------+
                +-----------+    |             +--| chromosome |
...
+ Switch from SQLite to MySQL backend: =useMySQL=.
...
+ _Example_: get protein annotations for a gene or filter genes by protein
  annotation.
  ...
  #+BEGIN_SRC R :results output pp
    ...
    ## Use protein annotations in filters
